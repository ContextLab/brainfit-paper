...
\newcommand{\frDetail}{S1}
\newcommand{\natDetail}{S2}
\newcommand{\vocabDetail}{S3}
\newcommand{\spatialDetail}{S4}
...
We expected (and found) that the general trends and
tendancies in the behavioral data followed previously reported
behaviors from similar tasks that had been utilized in prior work.  



In Figure~\ref{fig:immediate_behavior}, we summarize participants'
behaviors on four immediate (short term) memory tests: free recall
(Fig.~\ref{fig:immediate_behavior}a), a naturalistic recall task
(Fig.~\ref{fig:immediate_behavior}b), a foreign language vocabularly
learning task (Fig.~\ref{fig:immediate_behavior}c), and a spatial
learning task (Fig.~\ref{fig:immediate_behavior}d).
...
\caption{\textbf{Immediate memory tests.}}
...
\caption{\textbf{Delayed memory tests.}}
